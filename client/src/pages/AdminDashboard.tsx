...
export default function AdminDashboard() {
  ...
  const filteredAdminSubcategories = useMemo(() => {
    ...
    return subcategories.filter(
      (subcategory) => subcategory.category_id === subcategoryFilterCategoryId
    );
      ...
      <div style={containerStyle}>
        ...
        <div style={cardStyle}>
          ...
          >
            ...
            >
              <option value="">Filtrar lista de subcategorias por categoria</option>
              {categories.map((category) => (
                <option key={category.id} value={category.id}>
                  {category.name}
                </option>
            ...
            <button
              type="button"
              onClick={() => setSubcategoryFilterCategoryId("")}
              style={buttonSecondaryStyle}
            >
              Mostrar todas
            </button>
          ...
          {filteredAdminSubcategories.length === 0 ? (
            ...
            >
              {filteredAdminSubcategories.map((subcategory) => (
                <div
                  key={subcategory.id}
                  style={{
                    border: "1px solid #d9dee7",
