    ...
    <>
      <nav
        className={`sticky top-0 z-50 transition-all duration-300 ${
          isScrolled ? "bg-black shadow-lg shadow-black/40" : "bg-black"
        } border-b border-gray-800`}
      >
        <div className="container">
          <div className="flex items-stretch justify-between">
            {/* Logo - maior e com mais presença */}
            <Link href="/" className="flex items-center py-3 group">
              <img
                src="/logo.png"
                alt="Loc7 Equipamentos"
                className="h-14 sm:h-16 md:h-20 lg:h-[92px] w-auto object-contain transition-transform duration-300 group-hover:scale-[1.02]"
              />
            </Link>
                ...
                <div className="hidden md:flex items-center gap-16 justify-center flex-1 relative overflow-visible">
                  {navLinks.map((link) => (
                    ...
                    >
                      {link.hasDropdown ? (
                        <button
                          className={`loc7-nav-link flex items-center gap-1 text-sm font-medium text-white hover:text-gray-300 transition ${
              ...
              {submenuCategories.map((cat) => {
                ...
                return (
                  <Link
                    key={cat.name}
                    href={cat.href}
                    className="flex items-center gap-2 px-3 py-2 text-white hover:text-gray-300 transition-all whitespace-nowrap text-xs rounded"
                  >
