...
export function setupAdminAuthRoutes(app: Express) {
  ...
  // POST /api/admin/forgot-password - Recuperar senha
  app.post('/api/admin/forgot-password', (req, res) => {
    try {
      ...
      if (!user) {
        return res.json({ success: true, message: 'Se o email existe, você receberá um link de recuperação' });
      }

      // Gerar token de reset
      const resetToken = crypto.randomBytes(32).toString('hex');
      console.log(`Token de reset gerado para ${email}: ${resetToken}`);
      console.log(`Link de reset: /admin-reset-password?token=${resetToken}`);
  ...
  // POST /api/admin/reset-password - Resetar senha
